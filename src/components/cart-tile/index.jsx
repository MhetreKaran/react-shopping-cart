import React from "react";
import { useDispatch } from "react-redux";
import { removeFromCart } from "../../store/cartSlice";

function CartTile({ cartItem }) {
    const dispatch = useDispatch();
    const handleRemovetoCart=()=>{
        dispatch(removeFromCart(cartItem?.id))
    }
  return (
    <>
      <div className="flex items-center p-5 justify-between bg-red-500 mt-2 mb-2 rounded-xl">
        <div className="flex p-3">
          <img src={cartItem?.image} className="h-28 rounded-lg" />
          <div className="ml-10 self-start space-y-5">
            <h1 className="text-xl text-white font-bold">{cartItem?.title}</h1>
            <p className="text-white font-extrabold">{cartItem?.price}</p>
          </div>
        </div>
        <div className='flex item-center justify-center w-full mt-5'>
        <button onClick={handleRemovetoCart} className='bg-green-900 text-white border-1 rounded-lg font-bold p-4'>{'Remove to Cart'}</button>
      </div>
      </div>
    </>
  );
}

export default CartTile;
